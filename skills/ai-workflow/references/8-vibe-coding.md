...
5. **Verify (the gates).** Before anything integrates, the increment must pass concrete gates — make them non-negotiable and automated. And the evidence must be *real*: a slice is "done" only on actual test/CI output you ran and saw, exercising the code for real — never a claim, and never a vacuous placeholder test that passes without touching the behavior.
   ...
   - **Blast radius** — for a code-touching increment, the gate stays closed until the callers and references of every changed symbol were actually queried — the code graph where present, else grep — and the impacted set recorded in the spec or `PROGRESS.md`. This operationalizes "consult the code graph before touching code" into a checkable line-item: no change merges blind to what it reaches. Flexed by stakes — a light glance for `throwaway`, a real, recorded impact sweep for `platform`.
   ...
   - **Design standards** — for any slice with UI, it conforms to `docs/brand/DESIGN-STANDARDS.md`: mobile-first and reused components (no reinvented primitives), motion on every appropriate element via the chosen library, respecting `prefers-reduced-motion` and the Core Web Vitals budget, app-like navigation transitions, input masks + correct mobile keyboards on form fields, lazy + blurhash media, skeleton loaders / optimistic UI, theming via tokens (dark mode if in scope), a single consistent iconography set, PWA installability if scoped, and i18n-ready layout and copy. Route the UI work through `/ui-ux-pro-max:ui-ux-pro-max` (or the noted fallback). Record the pass (checklist items met / items waived with reason) in the slice's spec or `PROGRESS.md`. Flexed by stakes — visual inspection + Playwright screenshot per screen for `mvp`; Playwright navigation-transition assertions + Lighthouse CWV budget check for `platform`.
   ...
   - **Prune the bulky evidence once its conclusion is recorded.** Verification produces large, re-fetchable outputs — full file reads, grep dumps, raw build/test logs — that have served their purpose the moment their verdict is captured. Honoring the write-barrier (record the conclusion in `PROGRESS.md`/`STATE.md` *before* dropping the raw output), condense them: a build/test log collapses to **structured failures only** (file:line:message plus pass/fail counts), and a large read collapses to a one-line handle + its path. Never prune `STATE.md`, `GATES.md`, `PROGRESS.md`, or spec content — those are durable, not re-fetchable noise.
...
**One increment, one context.** Stage 8 is the only unbounded-length part of the pipeline, so the increment boundary is also the natural context reset. Start each increment in a **fresh context** rather than dragging one bloated thread across dozens of increments — a long thread accumulates context-rot and a creeping "context anxiety" that pushes a tired session to declare *done* before the criteria are actually met. Seed the fresh context only from the durable files — `docs/STATE.md`, `docs/GATES.md`, `PROGRESS.md`, the increment's `specs/` entry, and the git log — and run a quick baseline smoke check before writing any new code. The loop above is autonomous *between gates*, so the reset point is the increment boundary, not a mid-increment compaction: cross the review gate, then begin the next increment clean.
...
- [ ] Every integrated increment passed its gates (tests, lint, type-check, **product-security review**, **accessibility**, and **design-standards conformance** (`DESIGN-STANDARDS.md`) for UI slices) on **real, code-exercising output you ran and saw** — no asserted or placeholder passes — and cleared code review. (Product-security, a11y, and design-standards are flexed by stakes; SAST/SCA and abuse-case checks are must-be-green for `platform`/regulated.)
- [ ] All UI work routed through `ui-ux-pro-max` (or the degraded fallback recorded in `DESIGN-STANDARDS.md`).
